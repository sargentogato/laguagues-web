const why = {
  title:            'Por qué Languages Team',
  titleLanguage:    'Language Team',
  titleTransversal: 'Método Transversal',
  titleForWhom:     'Para Quién',
  titleObjective:   'Objetivos',
  titleResults:     'Resultados',
  languageTeam:
    'Somos un grupo de profesores nativos certificados, actores y cocineros unidos para enseñar de la manera más práctica posible.',
  transversalMethods: `desde un curso clásico de idiomas hasta clases de cocina y teatro en el idioma escogido en una misma hora. 
    Se trata de impregnarse del idioma. clases individuales, de grupo o personalizadas.`,
  forWhom:   'Todo el mundo, todas las edades, sin limites ',
  objective: `Simular la vida real y aprender con inmersión directa en la práctica tanto para los cursos regulares como para nuestros cursos más exóticos.`,
  results:   `Acercamiento más eficaz al idioma y a la cultura, mayor disfrute de las clases, mayor outcome, resultados, mayor confianza a la hora de aplicar 
      los conocimientos adquiridos.`,
} as const;

export default why;
